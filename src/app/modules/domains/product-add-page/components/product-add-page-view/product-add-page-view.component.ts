...
export class ProductAddPageViewComponent implements OnInit, AfterViewInit {
  ...
  constructor(private fb: FormBuilder, private router: Router, private productService: ProductService, private cdr: ChangeDetectorRef) {
  }
  ...
  createProductItem(product) {
    this.productService.createProductItem(product).subscribe(() => {
  ...
  editProductItem(product) {
    this.productService.editProductItem(product).subscribe(() => {
        this.router.navigate(['/product-list']);
      },
      error => {
        console.log(error);
